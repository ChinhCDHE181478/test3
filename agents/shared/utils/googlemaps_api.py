...
class GoogleMapsAPI:
    ...
    def __init__(self, api_key: str | None, rapidapi_host: str | None) -> None:
        ...
        if not self._aiosession:
            ...
            self._aiosession = aiohttp.ClientSession(
                headers={
                    ...
                    "X-RapidAPI-Key": str(self._api_key),
                    "X-RapidAPI-Host": str(self._rapidapi_host),
                },
    ...
    async def search_attractions(
        ...
        try:
            async with self._aiosession.post(
                url=f"https://{self._rapidapi_host}/v1/places:searchText",
                json={
    ...
    async def search_restaurants(
        ...
        try:
            ...
            if "places" in data:
                for place in data["places"]:
                    # Map place fields directly; do not aggressively filter by types.
                    types = place.get("types", [])
                    restaurant = {
                        "id": place.get("id"),
                        "name": place.get("displayName", {}).get("text", "Unknown"),
